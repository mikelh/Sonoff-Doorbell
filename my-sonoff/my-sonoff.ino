...
#ifdef USE_DFPLAYER
  else if ((pin[GPIO_DFP_TXD] < 99) && DFPlayerCommand(type, index, dataBuf, data_len, payload)) {
  // else if (DFPlayerCommand(type, index, dataBuf, data_len, payload)) {
    // Serviced
  }
...
//  AddLog(LOG_LEVEL_DEBUG);
  ...
  if (pin[GPIO_DFP_TXD] < 99) {
    DFPlayer_init();
  }
#else
    baudrate = 9600; // DFPlayer uses 9600 Baud fix
#endif // USE_DFPLAYER_SOFTSERIAL
#endif
